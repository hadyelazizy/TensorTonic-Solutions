...
def normal_distribution(mu, sigma, x):
    ...
    prob_1_std=float(stats.norm.cdf(mu+sigma, mu, sigma))-float(stats.norm.cdf(mu-sigma, mu, sigma))
    return {"z_score": z_score, "cdf": cdf, "pdf": pdf, "prob_within_1_std": round(prob_1_std,4)}
